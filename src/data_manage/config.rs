...
impl Display for ReadError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ReadError::SerdeJsonE(e) => {write!(f, "{}", e)}
            ReadError::IoE(e) => {write!(f, "{}", e)}
            Custom(e) => {write!(f, "{}", e)}
        }
    }
}
...
impl Default for Config {
    fn default() -> Self {
        Self { ip: IpAddr::from([127, 0, 0, 1]), proxy: Proxy(7878),times: None, suffix: ".yaml".into() }
    }
}
impl Visitor<'_> for ProxyVisitor {
    ...
    fn visit_u16<E>(self, v: u16) -> Result<Self::Value, E> where E: Error {
        if v == 0 { return Err(E::custom("端口不能是0")) }
        Ok(Proxy(v))
    }
    fn visit_u32<E>(self, v: u32) -> Result<Self::Value, E> where E: Error {
        if v >= 65536 { return Err(E::custom("端口过大")) }
        self.visit_u16(v as u16)
    }
